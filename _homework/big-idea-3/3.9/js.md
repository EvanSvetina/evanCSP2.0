...
permalink: /homework/big-idea-3/3.9/js
---
...
function checkSpeedLimit(currentSpeed, speedLimit) {
    const isSafe = currentSpeed <= speedLimit;  //less than or equal to speed limit
...
function compareCarSpeeds(speed1, speed2) {
    const fasterCar = speed1 > speed2 ? "Car 1" : "Car 2";  // its the simple if statement thing
...
function daddyPigDriving() {
    ...
    const shouldSlowDown = currentSpeed <= schoolZoneLimit;  // Daddy pig is drunk driving
    
    if (shouldSlowDown) {
        console.log("🐷 DRUNK daddy Pig says: 'Oops! I should SPEED UP'");
    } else {
        console.log("🐷 Drunk daddy Pig says: 'Perfect speed for the school zone!'");
    }
}
...
function playgroundEntryChecker() {
    ...
    if(oldEnough === true || youngEnough === true)   {
        canEnter = true;
    } else {
        canEnter = false;
    }
...
function suzyGameChooser(weather, hasKite) {
    ...
    if (weather === "sunny") {
        if (hasKite) {
            ...
            activity = "Hide and Seek"; // Fill in what to do without a kite
        }
    } else if (weather === "rainy") {
        activity = "Coloring books"; // Fill in rainy day activity
    } else {
        activity = "Reading"; // Fill in default activity
    }
